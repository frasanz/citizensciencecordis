import { recomputar, PROGRAMAS, ETIQUETA_PROGRAMA } from './src/compute.js';
...
const COLOR = { HORIZON: 'var(--he)', H2020: 'var(--h2020)', FP7: 'var(--fp7)', FP6: 'var(--fp6)' };
...
function construirFiltros(disponibles) {
  ...
  el('#filtros').innerHTML = `
    <div class="grupo"><span>Programa marco${boton("filtroPrograma")}</span><div class="opciones">${
      disponibles.map((c) => {
...
function aplicar() {
  ...
  estado.vista = recomputar(estado.base, {
    programas: estado.programas, periodo: estado.periodo, paisFoco: estado.paisFoco,
  });
  pintar();
}
...
function pintar() {
  ...
  el('#fuente').innerHTML = 'Fuente: datos abiertos de CORDIS'
  ...
  el('#cifras').innerHTML = [
    ...
    tile(num(s.organizacionesUnicas), 'entidades participantes', `${num(s.participaciones)} participaciones`, false, 'entidades'),
  ...
  tabla('#p-familias', 'Subprogramas', 'Nombres oficiales de CORDIS, unificados entre programas marco.', 'subprogramas',
  ...
  tabla('#p-entidades', `Entidades más activas · ${pais}`,
    ...
    r.entidadesFoco.slice(0, 14), (e) => [
      `<span class="nom" title="${esc(e.nombre)}">${esc(e.siglas || e.nombre)}</span>`,
...
const AYUDA = {
  ...
  entidadesFoco: ['Entidades más activas',
    ...
    '<b>Coordina</b> es en cuántos de ellos lidera. El nombre corto es el que da CORDIS; el completo aparece al posar el ratón.'],
  tablaProyectos: ['Proyectos',
    'La lista completa que cumple el criterio. Pulsa cualquier cabecera para ordenar, y el acrónimo para abrir la ficha oficial en CORDIS.',
  ...
  filtroPrograma: ['Programa marco',
    'Los programas con los que la UE financia la investigación, sucesivos en el tiempo: <b>FP7</b> (2007-2013), <b>Horizonte 2020</b> (2014-2020) y <b>Horizonte Europa</b> (2021-2027).',
    'Vienen los tres marcados. Desmárcalos para aislar una etapa: la comparación entre programas dice bastante, porque el peso de cada país cambia de uno a otro.',
...
function columnasProyectos(pais) {
  return [
    { id: 'acronimo', t: 'Acrónimo', v: (p) => p.acronimo || p.id, izq: true,
      c: (p) => `<a href="https://cordis.europa.eu/project/id/${p.id}" target="_blank" rel="noopener">${esc(p.acronimo || p.id)}</a>` },
  ...
  el('#p-proyectos').innerHTML = `
    <div class="cabecera-tabla">
      <div><h2>Proyectos${boton("tablaProyectos")}</h2>
        <p class="nota">Los ${num(orden.length)} proyectos que cumplen el criterio.
    ...
    <div class="tabla-scroll"><table><thead><tr>${
      cols.map((c) => `<th class="ord${c.id === col ? ' activa' : ''}${c.id === col && dir === -1 ? ' desc' : ''}"
        data-col="${c.id}">${esc(c.t)}</th>`).join('')}</tr></thead>
      <tbody>${vistos.map((p) => `<tr>${
        cols.map((c) => `<td class="${c.izq ? 'izq' : ''}">${c.c(p)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>
  ...
  el('#p-proyectos').querySelectorAll('[data-col]').forEach((th) => {
    th.addEventListener('click', () => {
      ...
      estado.orden = estado.orden.col === id
        ...
        : { col: id, dir: ['inicio', 'fin', 'aportacion', 'foco'].includes(id) ? -1 : 1 };
      tablaProyectos();
    });
  });
